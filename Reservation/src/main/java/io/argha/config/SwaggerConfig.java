...
public class SwaggerConfig {
	...
	public Docket productApi() {
		return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("io.argha"))
				.paths(regex("/irctc/v1/.*")).build().apiInfo(metaInfo());
	}
	
	private ApiInfo metaInfo()
	{
		ApiInfo apiInfo = new ApiInfo("Mini IRCTC(Backend assignment)", "Basic APIs for an IRCTC like system(Train Reservation)", "1.0", "Terms of service",
